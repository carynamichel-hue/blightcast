/* BlightCast service worker — makes the hosted app installable and lets it
   OPEN with no signal (last-cached app shell + whatever data is in the
   browser; weather refreshes when connectivity returns).
...
self.addEventListener('activate', function (e) {
  e.waitUntil(caches.keys().then(function (keys) {
    return Promise.all(keys.filter(function (k) { return k.indexOf('blightcast-') === 0 && k !== CACHE; })
